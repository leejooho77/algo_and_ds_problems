...
public class problem_0128 {
    ...
    public int lengthLongestPath(String input) {
        int max = 0, pathSize = 0, level = 0;
        // split array with \n and \t
        String[] array = input.split("\n|\t");
        Deque<Integer> stack = new ArrayDeque<>();
        for(int i = 0; i < array.length; i++) {
            int len = array[i].length();
            // if we meet empty string increase the level
            if(len == 0) {
                level++;
            }
            else {
                // if we have elements more than level, pop them
                while(stack.size() > level) {
                    pathSize -= stack.pop();
                }
                // push current length and increment pathSize by current length
                stack.push(len + 1);
                pathSize += len + 1;
                // if we find file in directory, update max
                if(array[i].contains("."))
        ...
        // if max is 0, we didn't have file, so return 0
        // else return max - 1
